"use server";
import { redirect } from "next/navigation"; import { clearAdminSession, isAdmin, setAdminSession } from "@/lib/auth"; import { savePost } from "@/lib/posts";
...
export async function publish(data:FormData){if(!(await isAdmin()))redirect("/admin");await savePost({title:String(data.get("title")),excerpt:String(data.get("excerpt")),topic:String(data.get("topic")),content:String(data.get("content")),coverImage:String(data.get("coverImage")||"")||undefined,coverAlt:String(data.get("coverAlt")||"")||undefined,status:data.get("status")==="published"?"published":"draft"});redirect("/admin?saved=1");}
