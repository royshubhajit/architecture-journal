...
export function ArticleEditor() {
  ...
  const [content,setContent] = useState(""); const [cover,setCover] = useState<UploadResult|null>(null);
  const [coverAlt,setCoverAlt] = useState(""); const [busy,setBusy] = useState<"cover"|"inline"|null>(null); const [error,setError]=useState("");
  ...
  return <form action={publish} className="editor">
    ...
    <label>Title<input name="title" required placeholder="A precise, useful title"/></label>
    <div className="two"><label>Topic<input name="topic" required placeholder="Distributed systems"/></label><label>Status<select name="status"><option value="draft">Draft</option><option value="published">Published</option></select></label></div>
    <label>Summary<textarea name="excerpt" required rows={3} placeholder="What will the reader understand?"/></label>
    ...
    <div className="editor-actions"><label className="inline-upload">{busy==="inline"?<LoaderCircle size={17} className="spin"/>:<ImagePlus size={17}/>} {busy==="inline"?"Uploading…":"Insert image at cursor"}<input type="file" accept="image/jpeg,image/png,image/webp,image/gif" onChange={e=>addInline(e.target.files?.[0])}/></label><button>Save post</button></div>
  </form>;
}
